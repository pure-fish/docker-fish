...
- Add a comment above any non-obvious command, complex conditional, or external tool invocation; avoid comments restating simple assignments
...
## Fish Syntax Differences from POSIX Shells

- Use `test` instead of `[[ ]]`, `set` instead of `=` for variable assignment, and `and`/`or` instead of `&&`/`||`
...
- Do not include a shebang line (e.g. `#!/usr/bin/env fish`) unless the script is meant to be run standalone outside Fish's autoload mechanism
